...
#
# Description: Beatiful map, see link
#
#################################################################################
...
#################################################################################
#
# load data stlat/stlong are the start points elat/elong are the end 
# points of the lines
#
#################################################################################
...
pwater <- c(geom_polygon(data = water, aes(x = long, y = lat, group = group), 
                       colour = "#708090", fill = "#708090"))

#create plot
plon2 <- plon1 + pbuilt + pwater + 
  geom_segment2(aes(xend = elong, yend = elat, size = count, colour = count)) + 
  scale_size(range = c(0.06, 1.8)) +
  scale_colour_gradient(low = "#FFFFFF", high = "#FFFF33", space = "rgb") + 
  coord_equal(ratio = 1/cos(lon$elat[1]*pi/180)) + 
  quiet + 
  opts(panel.background = theme_rect(fill = "#404040"))
